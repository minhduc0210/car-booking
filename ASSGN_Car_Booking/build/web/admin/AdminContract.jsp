    ...
    <body>
                    ...
                    <tbody>
                        <c:forEach items="${contractList}" var="contract">
                            <tr>
                                ...
                                <td>
                                    <button type="button" class="btn btn-secondary btn-sm" data-bs-toggle="modal" data-bs-target="#carModal_${contract.contractID}">Details</button>
                                </td>
                                <td class="text-center">
                                    <c:if test="${contract.status eq 'Pending'}">
                                        <button type="button" class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#acceptModal_${contract.contractID}">Accept</button>
                                        <button type="button" class="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#rejectModal_${contract.contractID}">Reject</button>
                                        <br>
                                        <form class="mt-1" action="AdminContract" method="post">
                                            <input type="hidden" name="id" value="${contract.contractID}" />
                                            <input type="hidden" name="action" value="update" />
                                            <input type="hidden" name="status" value="Deposited" />
                                            <button type="submit" class="btn btn-primary btn-sm">Deposited</button>
                                        </form>
                                    </c:if>
                                </td>
                            </tr>
        ...
        <script>
            $(document).ready(function () {
                $('#datatable').DataTable({
                    "lengthChange": false, // Disables the length change option
                    "pageLength": 5
                });
            });
        </script>
